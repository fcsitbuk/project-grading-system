...
if(($_SERVER['REQUEST_METHOD'] == "GET") || ($_SERVER['REQUEST_METHOD'] == "POST")){
	...
	if(isset($_GET['stuid'])){
		...
		if($select->num_rows){
			...
			if($title != ""){
				$stu_detail = "<div class='wisss_result' title='click to assess student'><img src='$stu_pix' alt='student photo' title='$stu_name($reg_no)'/><div id='wisssr_detail'><p>$title</p><h5><span>reg. no.</span>$reg_no</h5><h5><span>name</span>$stu_name</h5><p><span>S/N</span>$s_no</p></div> </div> ";
		...
		$select = $db->query("SELECT * FROM gra_asessor WHERE stu_reg = '".$reg_no."' AND ass_name = '".$lec_name."' LIMIT 1") or die($db->error);
			if($select->num_rows){
				...
				if($ins){
					$msg ="<p class='uploadMsg' style='color: #444444; background: #EEFFEE; padding: 30px 0px; letter-spacing: 2px;'> <i class='fa fa-check text-success'>&nbsp; &nbsp;</i>Assessment record for $reg_no successfully modified!!! <br/><b>Redirecting...</b></p>";
					header("refresh:3; url= assessor");
				}else{
                ...
                <div class="wiss_assess" style="display: block;">
                    ...
                    <form id="wissa_form" method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>">
                        ...
                        <div class="wissaf_mark">
                            ...
                            <input type="submit" class="btn btn-info" name="save" value="save" id="save"/>
